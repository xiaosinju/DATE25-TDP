...
/// \return the indices of nets on the critical paths.
///
std::vector<std::vector<std::string> > _report_timing_nodes(
    ot::Timer& timer, int n) {
  ...
  for (auto& path : paths) {
    ...
    auto& pl = result.back();
    for (auto& point : path) {
      auto name = point.pin.name();
      
      pl.push_back(name);
    }
    // pl.push_back(std::to_string(path.slack));
  }
  return result;
}
...
auto _report_fep(ot::Timer& timer) {
  return timer.report_fep();
}
///
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  pybind11::class_< // This binding converts a unique pointer.
    ...
    >(m, "Timer")
      ...
      .def("report_fep", &DREAMPLACE_NAMESPACE::_report_fep)
      ;
  ...
  m.def("report_timing_nodes", &DREAMPLACE_NAMESPACE::_report_timing_nodes, "Report timing paths");
